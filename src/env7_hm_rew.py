...
class XArmEnv(GoalEnv):
    def __init__(self, use_demonstrations=True, max_episodes_for_demo=1000, is_eval=False):
        ...
        self._max_episode_steps = 4096
        # self.goal_space = spaces.Box(
        #     low=np.array([260, 190, 250, -170, -90, 0]), 
        #     high=np.array([270, 200, 260, -155, -80, 10]),
        #     dtype='float32'
        # )
        # 234 287 255 -143 -84 -26
        self.goal_space = spaces.Box(
            low=np.array([229, 282, 250, -148, -89, -31]), 
            high=np.array([239, 292, 260, -138, -79, -21]),
            dtype='float32'
        )
        ...
        self.error_countdown = 0  # (주석) 에러 발생 시 카운트다운
    ...
    def step(self, action):
        ...
        try:
            error_code = self.arm.set_position(*action, relative=True, wait=True)
            if error_code != 0:
                if self.error_countdown == 0:
                    self.error_countdown = 20
                raise Exception(f"Error code: {error_code}")


        except Exception as e:
            done = False
            print(f"Error during set_position: {e}")
            print(f"prev_achieved_goal: {prev_state['achieved_goal']}")
            if self.error_countdown > 0:
                self.error_countdown -= 1
                if self.error_countdown == 0:
                    print("Error countdown expired. Ending episode.")
                    done = True
            return prev_state, -10, done, False, {"error": e}
        ...
        if collision_detected:
            ...
            return curr_state, -50, False, False, {"collision": True}
    ...
    def reset(self, seed=None, maybe_options=None, options2=None):
        ...
        error_code = self.arm.get_err_warn_code()
        state = self.arm.get_state()[1]
        print('error_code:', error_code)
        print('state:', state)
        if any(code != 0 for code in error_code[1]): # error_code[1]에 0이 아닌 게 있으면 에러
            print(f"Error detected: {error_code[1]}")
            self.arm.clean_error()
            time.sleep(1)
            self.arm.motion_enable(enable=True)
            time.sleep(1)
            self.arm.set_state(0)  # IDLE 상태로 변경
            time.sleep(3)
            print("Error cleared. Continuing...")

        if state != 0:  # IDLE 상태가 아니면 IDLE 상태로 변경
            self.arm.set_state(0)
            time.sleep(1)
            print("Changing state to IDLE...")
            for _ in range(10):  # 최대 10번까지 확인
                state = self.arm.get_state()[1]
                if state == 0:
                    break
                time.sleep(1)  # 상태 전환 대기
        ...
        if self.use_demonstrations and self.current_episode < self.max_episodes_for_demo:
            if self.demonstrations is not None:
                if self.is_eval:
                    ...
                    demo_index = np.random.randint(0.7 * len(self.demonstrations))
                    # demo_index = np.random.randint(len(self.demonstrations)) # 너무 가까운 곳에서 시작하면 무의미한 학습이 될 수 있음
    ...
    def compute_reward(self, achieved_goal, desired_goal, info, action=None):
        ...
        # 리워드 초기화
        joint_penalty_weight = 3
        distance_reward_weight = 1 # 1
        orientation_reward_weight = 0.3
        direction_reward_weight = 10
        collision_penalty_weight = 10
        success_reward_weight = 10
        demo_position_reward_weight = 0.3
        demo_orientation_reward_weight = 1
        action_penalty_weight = 0.001
        state_change_penalty_weight = 0.01
        consistency_reward_weight = 100
        total_reward = 0.0
        ...
        try:
            ...
            for angle, (min_limit, max_limit) in zip(joint_angles, JOINT_LIMIT):
                ...
                if angle < 0.95 * min_limit:
                    joint_penalty += -1 * joint_penalty_weight
                elif angle < 0.8 * min_limit:
                    joint_penalty += -0.2 * joint_penalty_weight
                # 각도가 최대 범위보다 클 경우 초과하는 만큼 페널티
                elif angle > 0.95 * max_limit: 
                    joint_penalty += -1 * joint_penalty_weight
                elif angle > 0.8 * max_limit:
                    joint_penalty += -0.2 * joint_penalty_weight
        ...
        max_position_distance = 880

        # 목표 거리 기반 보상
        
        distance_reward = distance_reward_weight * max(0, 1 - (position_distance / max_position_distance))  # 0 ~ 1 범위의 보상
        
        if position_distance < 0.01 * max_position_distance: # 0.01 * max_position_distance = 8.8
            distance_reward += 5
        elif position_distance < 0.05 * max_position_distance: # 0.05 * max_position_distance = 44
            distance_reward += 2
        elif position_distance < 0.1 * max_position_distance: # 0.1 * max_position_distance = 88
            distance_reward += 1
        
        total_reward += distance_reward
        
        # roll pitch yaw 각도 기반 보상
        orientation_reward = 0.0    
        # orientation_error = desired_goal[3:] - achieved_goal[3:]
        # orientation_error = ((orientation_error + 180) % 360) - 180
        # orientation_distance = np.linalg.norm(orientation_error)
        
        # max_orientation_error = [180, 180, 180]
        # max_orientation_distance = np.linalg.norm(max_orientation_error)
        
        # orientation_reward += orientation_reward_weight * max(0, 1 - (orientation_distance / max_orientation_distance))
        ...
        if action is not None and position_distance > 1e-3:
            ...
            if direction_norm > 1e-6:
                ...
                direction_reward += direction_reward_weight * np.dot(action[:3], direction_vector) / direction_norm
                ...
                if action_norm > 1e-6:
                    action_unit_vector = action[:3] / action_norm
        ...
        # demo position reward는 얼마나 demo position과 가까운지, demo directon reward는 demo들의 방향성을 action이 잘 따라가는지
        if self.demonstrations is not None:
            demo_weight = max(0, 1 - (self.current_episode - self.max_episodes_for_demo) / self.max_episodes_for_demo)
            if demo_weight > 0:
                demo_positions = self.demonstrations[:, :3]
                agent_pos = achieved_goal[:3]
                distances = np.linalg.norm(demo_positions - agent_pos, axis=1)
                demo_position_reward = demo_position_reward_weight * max(0, 1 - (min(distances) / max_position_distance))
                total_reward += demo_position_reward
                
                closest_index = np.argmin(distances)
                demo_similarity = 1 - (distances[closest_index] / max_position_distance)
                demo_orientations = self.demonstrations[closest_index, 4:7]
                
                demo_orientation_reward = 0.0
                # demo_orientation_error = demo_orientations - achieved_goal[3:]
                # demo_orientation_error = ((demo_orientation_error + 180) % 360) - 180
                # demo_orientation_distance = np.linalg.norm(demo_orientation_error)
                # demo_orientation_reward += demo_orientation_reward_weight * demo_similarity * max(0, 1 - (demo_orientation_distance / max_orientation_distance))
            
                total_reward += demo_orientation_reward
        ...
        if self.num_steps > 0:
            max_consistency_distance = np.linalg.norm(self.action_space.high[:3] - self.action_space.low[:3])
            prev_distance = getattr(self, 'prev_distance', max_position_distance)
            distance_diff = max(0.0, (prev_distance - position_distance) / max_position_distance)
            if position_distance < 1e-1:  
                distance_diff = 0.0

            consistency_reward = consistency_reward_weight * distance_diff
        
        total_reward += consistency_reward
        self.prev_distance = position_distance
        self.prev_action = action
        reward_dict = {'joint_penalty': joint_penalty, 'distance_reward': distance_reward,
                       'orientation_reward': orientation_reward, 'direction_reward': direction_reward,
                       'demo_position_reward': demo_position_reward, 'demo_orientation_reward': demo_orientation_reward,
                       'collision_penalty': collision_penalty, 'success_reward': success_reward,
                       'action_penalty': action_penalty, 'state_change_penalty': state_change_penalty,
                       'consistency_reward': consistency_reward, 'position_distance': position_distance}
        
        # 리워드 클리핑
        return total_reward.clip(-20, 20), reward_dict
